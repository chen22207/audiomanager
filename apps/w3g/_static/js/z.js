...
//点开选项菜单
	$(document).on('tap','.c,.c_replyme,.c_atme',function(){
		...
		if($(this).hasClass("c_replyme") || $(this).attr("type")=="comment"){
			$("#open_zf_twitter").addClass('hide');
			$("#digit").addClass('hide');
		}else{
			$("#open_zf_twitter").removeClass('hide');
			$("#digit").removeClass('hide');
		...
		// menu_box.show();
		menu_box.removeClass("menu_ani_hide").addClass("menu_ani_show").show().animate({'opacity':1},300);
		i=1;
		$(this).addClass("c_mousedown");
		shadow.show().addClass("shadow_show").removeClass("shadow_hide");
	});
//赞功能
...
$(document).on('tap','#shadow',function(){
	back2do="c.tap";
	menu_box.addClass("menu_ani_hide").removeClass("menu_ani_show");
	i=0;
	more_menu_box_option.hide().css({"opacity":0});
	default_menu_box_option.show().css({"opacity":1});
	$(".c_mousedown").removeClass("c_mousedown");
	setTimeout("menu_box.hide()",300);
	$(this).removeClass("shadow_show").addClass("shadow_hide");
	setTimeout("shadow.hide()",300);
});
	//打开菜单--更多
	menu_box.on('tap','#open_more',function(){
		...
		default_menu_box_option.animate({
			"opacity":0
			},50,"linear",function(){
			default_menu_box_option.hide();
			more_menu_box_option.show().animate({
					"opacity":1
				},50);
		});
	});
	...
	$(document).on('tap','#back2_default_menu_box_option',function(){
		more_menu_box_option.animate({
			"opacity":0
			},50,"linear",function(){
			more_menu_box_option.hide();
			default_menu_box_option.show().animate({
					"opacity":1
				},50);
		});
	});
	...
	post_twitter.on('tap','#close_post_twitter',function(){
	...
	$(document).on('tap','#menu_button',function(){
		if(menu_button_tap==0){
			part_shadow.show().addClass("shadow_show").removeClass("shadow_hide");
			sys_menu.show().removeClass("sys_menu_hide").addClass("sys_menu_active");
			if(if_scroll==1){
				$(this).css({
			...
			part_shadow.addClass("shadow_hide").removeClass("shadow_show");
			setTimeout("sys_menu.hide()",300);
			setTimeout("part_shadow.hide()",200);
			menu_button_tap=0;
		}
	});
	//用户选择评论+转发//转发+评论
	...
	$(document).on('tap','#post_twitter_button_submit',function(){
		...
		if(content.length>content_length_max){
			tips("微博超过"+ content_length_max +"字，请重新编辑后发送",1,0);
	...
	part_shadow.tap(function(){
		...
		sys_menu.removeClass("sys_menu_active").addClass("sys_menu_hide");
		part_shadow.removeClass("shadow_show").addClass("shadow_hide");
		setTimeout("sys_menu.hide()",300);
		setTimeout("part_shadow.hide()",300);
		menu_button_tap=0;
	});
	//菜单选择写微博
	...
	$(document).on('tap','#open_square',function(){
		menubox2skip(U('w3g/Index/publicsquare'),'载入微博广场中');
		$(".c_mousedown").removeClass("c_mousedown");
	});
	$(document).on('tap','#sys_menu_square',function(){
		sysmenu2skip(U("w3g/Index/publicsquare"),'载入微博广场中');
	});
	//退出登录
	...
	$(document).on('tap','.c_d_yes',function(){
		if($(this).hasClass("c_d_p")){//举报
			...
			var this_parent=this_c.parent();
			this_parent.hide();
			// this_c.parent().css('overflow','hidden').animate({"height":0},100,"linear",function(){$(this).remove();});
			$.ajax({
				type:"POST",
			...
			$.ajax({
				...
				success:function(r){
					if(Number(r.feed_id)>0){
						tiph();
						rst();
						var html = '<div id="weiba_box" style="top:'+st+'px"><div id="weiba_close">X</div><div id="weiba_title">'+r.title+'</div><div id="weiba_content">'+r.content+'</div></div>';
						$("body").append(html);
